...
const Dashboard: React.FC<DashboardProps> = ({ data }) => {
  ...
  const activeItems: InventoryItem[] = useMemo(() => {
    if (selectedBundleId === 'all') return data.items;
    ...
    <div className="p-4 pb-24 space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        ...
        <div className="relative">
            <select value={selectedBundleId} onChange={(e) => setSelectedBundleId(e.target.value)} className="w-full appearance-none bg-gray-50 border border-gray-200 text-gray-800 py-3 px-4 pr-8 rounded-lg leading-tight focus:outline-none focus:bg-white focus:border-brand-500 font-medium text-sm transition-all">
                <option value="all">📊 全部库存总览</option>
                <option disabled>──────────</option>
                {data.bundles.map(b => (<option key={b.id} value={b.id}>📦 {b.name}</option>))}
            </select>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gradient-to-br from-brand-500 to-brand-700 p-4 rounded-2xl shadow-lg shadow-brand-200 text-white">
            <span className="text-xs text-brand-100 opacity-80">{selectedBundleId === 'all' ? '总库存件数' : '组合内总件数'}</span>
            <span className="text-4xl font-bold mt-1 block">{totalStockCount}</span>
            <span className="text-xs text-brand-100 mt-2 block opacity-60">涉及 {activeItems.length} 款商品</span>
        </div>
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-center">
             ...
             <span className="text-3xl font-bold text-gray-800 mt-1 block">{data.bundles.length}</span>
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
        <div className="flex justify-between items-center mb-6">
            <h3 className="text-sm font-bold text-gray-800">{selectedBundleId === 'all' ? '全部库存尺码分布' : '当前组合尺码分布'}</h3>
        </div>
        <div className="h-56 w-full">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stockBySize} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
                    <XAxis dataKey="name" tick={{fontSize: 11, fill: '#6b7280'}} stroke="#e5e7eb" axisLine={false} tickLine={false} dy={10} />
                    <Tooltip cursor={{fill: 'transparent'}} contentStyle={{borderRadius: '8px', border:'none', boxShadow:'0 4px 12px rgba(0,0,0,0.1)'}} />
                    <Bar dataKey="value" radius={[6, 6, 6, 6]}>
                        {stockBySize.map((_entry, index) => <Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />)}
                        <LabelList dataKey="value" position="top" fill="#374151" fontSize={12} fontWeight="bold" formatter={(val: number) => val > 0 ? val : ''} />
                    </Bar>
                </BarChart>
            </ResponsiveContainer>
        </div>
